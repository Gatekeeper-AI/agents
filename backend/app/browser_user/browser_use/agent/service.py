...
class Agent:
	...
	async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
		...
		try:
			...
			"""print("Heres the good stuff:")

			print(state.url)
			if state.url not in self.visited and state.url != "about:blank":
				try:
					self.visited[state.url] = len(self.visited) + 1
					filename = f'{self.visited[state.url]}.json'
					...
					with open(filepath, 'w') as f:
						string_dict = {key: str(value) for key, value in state.selector_map.items()}
						print(f"Dictionary to write: {string_dict}")
						json.dump(string_dict, f, indent=2)
					
					print(f"File written successfully: {filepath}")
					
					with open('output/visited.json', 'w') as h:
						json.dump(self.visited, h, indent=2)
